...
export default function Invoicedetail() {
  ...
  const handleMarkDeposit = async () => {
    ...
    if (depositAmount > 0) {
      ...
      try {
        const response = await fetch('https://roofing-31jz.onrender.com/api/addpayment', {
          method: 'POST',
          headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      if ((savedDepositData != null || savedDepositData != "") && savedDepositData._id != undefined) {
        ...
        const response = await fetch(`https://roofing-31jz.onrender.com/api/updatedeposit/${savedDepositData._id}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': authToken,
        ...
        if (response.status === 401) {
          ...
          if (data.Success) {
            ...
            const savedDepositResponse = await fetch(`https://roofing-31jz.onrender.com/api/deposit/${data.deposit._id}`);
        ...
        const response = await fetch('https://roofing-31jz.onrender.com/api/deposit', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': authToken,
  ...
  const fetchinvoicedata = async () => {
    try {
      const userid = localStorage.getItem("userid");
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`https://roofing-31jz.onrender.com/api/getinvoicedata/${invoiceid}`, {
        headers: {
          'Authorization': authToken,
        }
      });
  ...
  const fetchdepositdata = async () => {
    try {
      const userid = localStorage.getItem("userid");
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`https://roofing-31jz.onrender.com/api/getdepositdata/${userid}/${invoiceid}`, {
        headers: {
          'Authorization': authToken,
        }
      });
  ...
  const fetchtransactiondata = async () => {
    try {
      const userid = localStorage.getItem("userid");
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`https://roofing-31jz.onrender.com/api/gettransactiondata/${invoiceid}`, {
        headers: {
          'Authorization': authToken,
        }
      });
...
.text-end {
  ...
  const handleRemove = async (invoiceid) => {
    const authToken = localStorage.getItem('authToken');
    try {
      const response = await fetch(`https://roofing-31jz.onrender.com/api/deldata/${invoiceid}`, {
        method: 'GET',
        headers: {
          'Authorization': authToken,
        }
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch('https://roofing-31jz.onrender.com/api/send-invoice-email', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        await fetch(`https://roofing-31jz.onrender.com/api/updateinvoicedata/${invoiceid}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': authToken,
  ...
  const handleDepositFormSubmit = async (event) => {
    ...
    const contentAsPdf = await generatePdfFromHtml();
    try {
      const finalContent = content.trim() || `Thank you for Business With Us</p>`; // If content is empty, use default value
      const response = await fetch('https://roofing-31jz.onrender.com/api/send-deposit-email', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
